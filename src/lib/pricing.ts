...
export type Loc = "SH" | "SA" | "VP" | "VH" | "VdM";
export const LOC_KEY: Record<string, Loc> = {
  "Santiago Hotel": "SH",
  "Santiago Airport": "SA",
  "Valparaíso Port": "VP",
  "Valparaíso Hotel": "VH",
...
// Guest tier mapping
export type Tier = "1" | "2" | "3" | "4" | "5" | "6-10" | "11-15";
export const getTier = (guests: number): Tier => {
...
export const PRICES: Record<string, Record<Tier, Record<string, number>>> = {
  classic: {
...
/** Lowest per-person price (in thousands of CLP) across every tier/route for a tour — used for "starts from" copy. */
export const getMinPrice = (tourId: string): number | null => {
  const tiers = PRICES[tourId];
  if (!tiers) return null;
  let min = Infinity;
  for (const routes of Object.values(tiers)) {
    for (const price of Object.values(routes)) {
